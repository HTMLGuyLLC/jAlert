/* 
	...
	*
*/
;(function($){$.fn.jAlert=function(options){var alert=this,themes=['default','green','red','black','blue'],sizes=['xsm','sm','md','lg','xlg','full'],backgroundColors=['white','black'],styles=[],classes=['animated'],backgroundClasses=[];if(alert.length>1){alert.each(function(){$(this).jAlert(alert.options);});return this;}
...
backgroundClasses.push('ja_'+alert.options.backgroundColor);if((typeof alert.options.btns=='object'||typeof alert.options.btns=='array')||alert.options.autofocus){alert.options.closeOnClick=false;}
...
{hideBackground();}},alert.options.animationTimeout);}
return this;}
...
if(alert.options.closeOnClick){$(document).off('mouseup');$(document).on('mouseup',function(e){$('.jAlert:visible').each(function(){var alert=$(this);if(alert.options.closeOnClick)
{alert.closeAlert();}});});}
if(alert.options.closeOnEsc){$(document).off('keydown');$(document).on('keydown',function(e){if(e.keyCode===27){$('.jAlert:visible').each(function(){var alert=$(this);if(alert.options.closeOnEsc)
{alert.closeAlert();}});}});}
